...
class VPulse_LIV():
    ...
    def start_liv_pulse(self):
        ...
        # Convert current and voltage readings to mA and mV values
        # Save a new list as the voltage readings in V
        voltageDataV = list()
        voltageDataV[:] = [x for x in voltageData]
        currentData[:] = [x*1000 for x in currentData]
        voltageData[:] = [x*1000 for x in voltageData]
        ...
        fig, ax1 = plt.subplots()
        ax2 = ax1.twinx()
        ax1.set_ylabel('Measured device light output (W)', color='red')
        ax1.set_xlabel('Measured device current (mA)')
        ax2.set_ylabel('Measured device voltage (V)', color='blue')
        ax2.plot(currentData, voltageDataV, color='blue', label='I-V Characteristic')
        ax1.plot(currentData, lightData, color='red', label='L-I Characteristic')
        ax1.legend(loc='upper left')
