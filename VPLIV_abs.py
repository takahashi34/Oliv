...
class VPulse_LIV():
    ...
    def start_liv_pulse(self):
        ...
        if self.computeAbsPower:
            ...
            R = float(self.responsivity_entry.get())

            C = self.precompute_constant(x, y, z, lam, Ad, R, Z)
            ...
            ax2.plot(currentData, absolute_power, color='red', label='I-P Characteristic')
            ...
            filename2 = self.device_name_entry.get() + '_VP-LIV_' + self.device_temp_entry.get() + \
                'C_' + self.device_dim_entry.get() + '_' + self.test_laser_button_var.get() + '_absPower'
            fig2.savefig(f"{self.plot_dir_entry.get()}/{filename2}.png")
            #plt.show()

            # open another file and write in absolute power data
            txtDir2 = self.txt_dir_entry.get()
            filepath2 = os.path.join(txtDir + '/' + filename2 + '.txt')
            fd2 = open(filepath2, 'w+')
            i = 1

            fd2.writelines('Device power output (P)\tCurrent (mA)\tVoltage (mV)\n')
            for i in range(0, len(currentData)):
                fd2.writelines(str(absolute_power[i]))
                fd2.writelines('\t')
                fd2.writelines(str(currentData[i]))
                fd2.writelines('\t')
                fd2.writelines(str(voltageData[i]))
                fd2.writelines('\n')
            fd2.close()
    ...
    def precompute_constant(self, x, y, z, lam, Ad, R, Z):
        PI = np.pi
        PI2 = PI * PI
        ...
        return (PI * x * y * term_x * term_y) / (2 * Ad * R * Z)
    ...
    def __init__(self, parent):
        ...
        self.plotFrame.grid(column=0, row=2, sticky='N')
        ...
        self.paramsFrame.grid(column=1, row=1, sticky='N', padx=(10,5), pady=(5,0))
        ...
        self.compute_power_checkbox.grid(column=4, row=0, columnspan=3, sticky='W', pady=(10,0))
